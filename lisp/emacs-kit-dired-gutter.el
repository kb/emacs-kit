;;; emacs-kit-dired-gutter.el --- Git status indicators in Dired buffers  -*- lexical-binding: t; -*-
;;
...
(use-package emacs-kit-dired-gutter
  :if emacs-kit-enable-dired-gutter
  :ensure nil
  :no-require t
  :defer t
  :init
  (setq emacs-kit-dired-gutter-enabled t)

  (defvar emacs-kit/dired-git-status-overlays nil
    "List of active overlays in Dired for Git status.")

  (defun emacs-kit/dired--git-status-face (code)
  ...
  (defun emacs-kit/dired-git-status-overlay ()
    ...
    (let ((git-root (ignore-errors (vc-git-root default-directory))))
      (when (and git-root
                 ...
                 emacs-kit-dired-gutter-enabled)
        ...
        (let* ((git-status (vc-git--run-command-string nil "status" "--porcelain" "--ignored" "--untracked-files=normal"))
          ...
          (mapc #'delete-overlay emacs-kit/dired-git-status-overlays)
          (setq emacs-kit/dired-git-status-overlays nil)
          ...
          (dolist (line (split-string git-status "\n" t))
            (when (string-match "^\\(..\\) \\(.+\\)$" line)
              (let* ((code (match-string 1 line))
                     ...
                     (status-face (emacs-kit/dired--git-status-face code)))
              ...
              (let* ((file (ignore-errors (expand-file-name (dired-get-filename nil t)))))
                (when file
                  ...
                  (let* ((status-face (gethash file status-map (cons "  " 'font-lock-keyword-face)))
                    ...
                    (push ov emacs-kit/dired-git-status-overlays))))
              (forward-line 1)))))))

  (add-hook 'dired-after-readin-hook #'emacs-kit/dired-git-status-overlay))

(provide 'emacs-kit-dired-gutter)
;;; emacs-kit-dired-gutter.el ends here
