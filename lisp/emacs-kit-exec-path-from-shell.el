;;; emacs-kit-exec-path-from-shell.el --- Sync shell PATH into Emacs  -*- lexical-binding: t; -*-
;;
...
(use-package emacs-kit-exec-path-from-shell
  :ensure nil
  :no-require t
  :defer t
  :init
  (defun emacs-kit/set-exec-path-from-shell-PATH ()
    ...
    (let* ((shell (getenv "SHELL"))
           ...
           (command
            (cond
             ...
             (t nil))))
      (if (not command)
          (message "emacs-kit: Unsupported shell: %s" shell-name)
        (let ((path-from-shell
          ...
          (when (and path-from-shell (not (string= path-from-shell "")))
            ...
            (message ">>> emacs-kit: PATH loaded from %s" shell-name))))))

  (add-hook 'after-init-hook #'emacs-kit/set-exec-path-from-shell-PATH))

(provide 'emacs-kit-exec-path-from-shell)
;;; emacs-kit-exec-path-from-shell.el ends here
