;;; emacs-kit-m3u.el --- M3U playlist viewer and online radio player  -*- lexical-binding: t; -*-
;;
...
(use-package emacs-kit-m3u-visualizer
  :ensure nil
  :no-require t
  :defer t
  :init
  (defvar emacs-kit/m3u-radio-sources
...
If RAW-BUFFER is nil, use the current buffer."
  ...
  (defun emacs-kit/get-online-radio-list-m3u ()
    "Select and download an online M3U playlist, then visualize it."
    (interactive)
    (let* ((choice (completing-read "Choose your Online Radio playlist: " emacs-kit/m3u-radio-sources))
           (url (cdr (assoc choice emacs-kit/m3u-radio-sources)))
...
`display' property. If it's already an image, restore the original URL text."
  ...
  (global-set-key (kbd "C-c r") #'emacs-kit/get-online-radio-list-m3u)
...
(provide 'emacs-kit-m3u)
;;; emacs-kit-m3u.el ends here
