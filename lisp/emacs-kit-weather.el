;;; emacs-kit-weather.el --- Weather forecast from wttr.in  -*- lexical-binding: t; -*-
;;
...
(use-package emacs-kit-weather
  :ensure nil
  :no-require t
  :defer t
  :init
  (setq emacs-kit-weather-city "Indaiatuba")

  (defun emacs-kit/weather-buffer (&optional which)
...
Optional WHICH:
  ...
  nil   → fetch both."
    (interactive)
    (let* ((city (shell-quote-argument emacs-kit-weather-city))
      ...
      (pcase which
        ('url1
         (emacs-kit--fetch-weather url1 buffer))
        ('url2
         (emacs-kit--fetch-weather url2 buffer t))
        (_
         (emacs-kit--fetch-weather url1 buffer)
         (emacs-kit--fetch-weather url2 buffer t)))))

  (defun emacs-kit--fetch-weather (cmd buffer &optional second)
...
(provide 'emacs-kit-weather)
;;; emacs-kit-weather.el ends here
